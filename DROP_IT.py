...
def plan_trials(path, request, context=None, blocks=1, seed=None, start_index=1,
    ...
    prepared = now_utc()
    header = {
        "type": "plan", "schema": "drop-it-study/4.0", "local_seq": 1,
    ...
    return {"study_id": header["study_id"], "seed": seed, "blocks": blocks,
...
def load_study(path):
    ...
    if not records or not isinstance(records[0], dict) or records[0].get("schema") not in ("drop-it-study/3.0", "drop-it-study/4.0"):
        raise ValueError("unsupported study schema")
    trials, outcomes = {}, {}
    if records[0]["schema"] == "drop-it-study/4.0":
...
class ReceiptLog:
    ...
    def capture(self, raw, stream=None, metadata=None, decoded_payload=None,
                ...
                metadata_source_sha256=None, **fields):
...
def make_canary(writer, stage, input_path=None, output_path=None, trial_id=None):
    if not stage.strip():
    ...
    token = "OZCANARY-" + secrets.token_hex(12).upper()
    ...
    if input_path is not None:
        ...
        receipt = writer.add("canary_created", token.encode("ascii"), **fields)
    return {"token": token, "stream": stage, "trial_id": trial_id,
...
def joined_delta_measurements(rows):
    ...
    for row in rows:
        if row["kind"] != "event" or not row.get("delta_fragments"):
            continue
        identifiers = row["metadata_observed"]["identifiers"]
        ids = [(item["field"], str(item["value"])) for item in identifiers
               if item["field"] in {"response_id", "responseId", "item_id", "itemId"}
               and item["value"] is not None]
        if not ids:
            continue
        for fragment in row["delta_fragments"]:
            key = (row.get("stream"), tuple(sorted(set(ids))), fragment["json_path"])
            groups.setdefault(key, []).append((row, fragment["text"]))
    result = []
    for (stream, identifiers, pointer), pieces in groups.items():
        ...
        result.append({"stream": stream, "correlation_ids": dict(identifiers),
...
def trace_report(path):
    ...
    return {"layer": "MEASUREMENT", "receipts": len(rows), "validated_payloads": len(checked),
            ...
            with ReceiptLog(args.log) as writer:
                if args.operation == "capture":
                    ...
                    result = import_capture(writer, args.input, args.format, args.stream, metadata,
                                            ...
                                            args.condition, args.prime_index, metadata_sha)
                    ...
                    result = make_canary(writer, args.stage, args.input, args.output, args.trial)
...
def main():
    ...
    try:
        ...
        if args.audit:
            ...
            if not args.json:
                result = {"trial_id": result["trial_id"], "status": "RECORDED",
