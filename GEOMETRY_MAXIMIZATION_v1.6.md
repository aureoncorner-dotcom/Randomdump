# Geometry Maximization v1.6

**Predictive phases, toroidal geometry, and density-valued differential calculus**

Integration of the additional `Mathamatics5 .txt` source · 6 September 2026

This complete revision retains the earlier predictive and geometric results. It adds a concrete density calculus on the smooth phase space, connects it to the established conformal measures, and derives the existing Virasoro cocycle through an explicit integration map.

- **Density transformation laws:** time action on arbitrary density weights is transported to the earlier Koopman action. Its weighted Hilbert norm uses the conformal measure with exponent twice the real part of the density weight (§39).
- **A specified comparison of Lie cochains:** integration of a weight-one density cocycle gives the existing scalar Virasoro class, with its normalization and coboundary explicit (§40.1–40.2).
- **Coverings through actual maps:** vector-field lifts and density pullbacks explain the 13 and 39 factors while distinguishing the other low-weight cocycles (§40.3).
- **A usable quadrature limit:** 39 uniform phase samples check the cocycle identities exactly for input modes through frequency 12; higher-frequency examples exhibit failure (§40.4).
- **Reproducible source corrections:** positive-Witt complexes, Verma versus vacuum Gram matrices, centralizer dimensions and shifted reflections are checked directly. The broader knot and categorical survey is qualified by its actual inputs (§41 and the source review).

Sections 1–38 retain the earlier mathematics and version-specific verification history. Sections 39–42 are the current additions. The new verifier replays v1.5, including its nested v1.4 checks, and the archive preserves every earlier package. Source-file instructions were treated as content. The new density structures do not specify a physical central charge, a vacuum representation or a knot invariant for the screen.

## 1. Source coverage reported in v1.0

**Historical retrieval record.** The counts, archive descriptions, and recovery attributions in this section are carried forward from v1.0. The v1.2 source snapshots and checks are identified in §24; the S40 additions and checks are in §29, the S41 additions and checks are in §34, the S42 additions and checks are in §38, and the current S43 additions and checks are in §42.
...
The v1.1 report, standalone verifier, executed JSON receipt, initial two source snapshots, README, and checksums are preserved in the v1_5/v1_4/v1_3/v1_2/v1_1 folder of the current archive. The same materials were delivered in `GEOMETRY_MAXIMIZATION_v1.1_verification.zip`. From the v1_1 folder, the earlier verifier can be run with Python 3.10 or later:
...
**Historical v1.2 receipt.** The package described in this section is preserved under v1_5/v1_4/v1_3/v1_2/ in the v1.6 archive. Its v1_1/ paths are relative to that preserved package. The current checks are in §42.
...
**Historical v1.3 receipt.** The archive and relative paths described in this section refer to the preserved package now under v1_5/v1_4/v1_3/. The current v1.6 source and verification record are in §42.
...
**Historical v1.4 receipt.** The archive and relative paths described below refer to the preserved package under v1_5/v1_4/. Its original mathematics and receipt remain intact. The current v1.6 checks, including a fresh replay of v1.4, are in §42.
...
## 38. Retained v1.5 source review, verification, and resulting status

**Historical v1.5 receipt.** The archive and relative paths below refer to the preserved package under v1_5/. The current source review and execution record, including a fresh replay of this version, are in §42.
...
## 39. Tensor densities on the predictive phase

The new source, S43, develops cohomology with density coefficients. This supplies a concrete extension of the smooth calculus in §35: specify how an observable changes when its coordinate changes, then retain that rule in the derivative, time action and phase coverings.

### 39.1 Coordinate transformation and time action

Retain the smooth diffeomorphism \(h\), the map \(F=h^{-1}R_\alpha h\), the normalized derivative \(D=D_h\), and \(I=I_h\) from §35. In particular \(h'>0\). Write \(\theta=h(x)\) and \(d\theta=h'(x)dx\). A smooth complex \(\lambda\)-density can be expressed as
...
For noninteger or complex \(\lambda\), the power uses the real logarithm of the positive function \(h'\). This is a transformation rule for sections, not a claim that arbitrary fractional powers of differential forms use ordinary exterior multiplication.

The natural pullback by the actual time step is

\[
T_\lambda a=(F')^\lambda(a\circ F).
\]

Since \(h'(F)F'=h'\), it becomes the ordinary rotation pullback in the phase frame:

\[
T_\lambda\bigl((h')^\lambda\widetilde a\bigr)
=(h')^\lambda K_F\widetilde a.
\]

Thus every density weight has an explicitly defined time action, with the same phase frequencies. In the original coefficient frame the eigenvectors are

\[
a_n^{(\lambda)}=(h')^\lambda e_n^h,
\qquad T_\lambda a_n^{(\lambda)}=e^{2\pi i n\alpha}a_n^{(\lambda)}.
\]

For arbitrary diffeomorphisms, pullback reverses composition: \((G\circ H)^*=H^*G^*\). For the iterates of this single \(F\), the formulas give the usual action of integer time. The density action itself is standard; see [Ovsienko–Roger, §1](https://arxiv.org/pdf/hep-th/9409067). Its realization using the established \(h\), occupation and predictive phases is specified here.

### 39.2 A direct connection to the earlier conformal measures

Equip coefficient functions with the norm

\[
\|a\|_\lambda^2=\int_0^1|a(x)|^2(h'(x))^{1-2\operatorname{Re}\lambda}\,dx.
\]

Multiplication \(M_\lambda\widetilde a=(h')^\lambda\widetilde a\) is a unitary map from \(L^2(\mu_0)\) to this weighted space, because

\[
\|M_\lambda\widetilde a\|_\lambda^2
=\int_0^1|\widetilde a|^2h'\,dx=I(|\widetilde a|^2).
\]

The conjugacy \(T_\lambda M_\lambda=M_\lambda K_F\) proves that \(T_\lambda\) is unitary for this norm. This is a statement about the specified time action; it does not declare every diffeomorphism unitary for a fixed \(h\)-dependent norm.

The measure in the norm is precisely the unnormalized conformal measure of §27 with

\[
s=2\operatorname{Re}\lambda,\qquad
d\mu_s=Z_s^{-1}(h')^{1-s}\,dx.
\]

For example, weight zero gives ordinary Koopman observables in \(L^2(h'dx)\); half-densities give the Jacobian-weighted action in ordinary \(L^2(dx)\); weight one gives \(L^2((h')^{-1}dx)\). The earlier negative-one conformal measure corresponds here to real density weight \(-1/2\). Its separate role in the tangent functional of §32.2 still requires that section's normalization.

This connection uses strictly positive \(h'\). It does not extend these Hilbert-space formulas unchanged to a coordinate with critical zeros.

### 39.3 Infinitesimal action, duality and projection

Identify \(X_f=fD\) with its coefficient in the phase frame. Its bracket is \([f,g]_D=fDg-gDf\). On phase-frame density coefficients define

\[
\rho_\lambda(f)a=fDa+\lambda(Df)a.
\]

The product rule gives

\[
[\rho_\lambda(f),\rho_\lambda(g)]=\rho_\lambda([f,g]_D).
\]

For the Witt sign convention of §36 this means

\[
\rho_\lambda(L_m)e_n^h=-(n+\lambda m)e_{m+n}^h.
\]

In the physical \((dx)^\lambda\) frame, the same action is the usual Lie derivative by the vector field \(f/(2\pi i h')\,\partial_x\). Transporting to the phase frame cancels the derivatives of \(h'\) and produces the displayed formula. Time covariance follows from \(DK_F=K_FD\).

Multiplication of coefficient functions realizes the density product \(\mathcal F_\lambda\otimes\mathcal F_\nu\to\mathcal F_{\lambda+\nu}\). The pairing

\[
\langle a,b\rangle=I(ab),\qquad
a\in\mathcal F_\lambda,\quad b\in\mathcal F_{1-\lambda},
\]

is infinitesimally invariant: the two action terms sum to \(I(D(fab))=0\). In particular the smooth, or regular, dual of the vector-field module \(\mathcal F_{-1}\) is \(\mathcal F_2\). This corrects S43's identification of the regular coadjoint module with \(\mathcal F_1\); the full continuous dual also includes distributions.

The previous finite-fiber expectation becomes an orthogonal projection on the weighted coefficient space:

\[
P_{\lambda,k}=M_\lambda E_{k,h}M_\lambda^{-1},\qquad k=13,39.
\]

It projects onto the span of \((h')^\lambda e_{kn}^h\), commutes with time, and has norm one. Orthogonality follows from finite rotation averaging in \(L^2(\mu_0)\). It does not assert preservation of products or of the action of every vector field. For example at weight zero,

\[
E_{k,h}\rho_0(e_1^h)e_{-1}^h=-1,
\qquad \rho_0(e_1^h)E_{k,h}e_{-1}^h=0.
\]

These projections retain a continuous phase and do not produce a finite-dimensional predictive generator.

## 40. Density-valued cocycles and an explicit integration map

### 40.1 Low-weight cocycles with their coefficient modules supplied

Use the usual Chevalley–Eilenberg differential. For a one-cochain \(A\) with values in \(\mathcal F_\lambda\),

\[
(dA)(f,g)=\rho_\lambda(f)A(g)-\rho_\lambda(g)A(f)-A([f,g]_D).
\]

The following are continuous one-cocycles, as direct differentiation verifies:

\[
f\mapsto f\in\mathcal F_0,\quad
f\mapsto Df\in\mathcal F_0,\quad
f\mapsto D^2f\in\mathcal F_1,\quad
f\mapsto D^3f\in\mathcal F_2.
\]

Taking alternating products of these one-cocycles using density multiplication gives five local two-cocycles:

\[
\begin{aligned}
\overline c_\lambda(f,g)&=fD^{\lambda+1}g-gD^{\lambda+1}f,
&&\lambda=0,1,2,\\
c_\lambda(f,g)&=(Df)D^{\lambda+1}g-(Dg)D^{\lambda+1}f,
&&\lambda=1,2.
\end{aligned}
\]

Each displayed output is a coefficient in \(\mathcal F_\lambda\). These are specific representatives from the low-weight density calculation of [Ovsienko–Roger, Theorem 3](https://arxiv.org/pdf/hep-th/9409067). They are not a claimed list of all cochains or all density weights. The integrated constant-valued class at weight zero is a further, distinct representative.

The two-cocycle identity, including the module action, is

\[
\begin{aligned}
0={}&\rho_\lambda(f)c(g,u)-\rho_\lambda(g)c(f,u)
+\rho_\lambda(u)c(f,g)\\
&-c([f,g]_D,u)+c([f,u]_D,g)-c([g,u]_D,f).
\end{aligned}
\]

A cocycle supplies the bracket

\[
[(f,a),(g,b)]=([f,g]_D,\rho_\lambda(f)b-\rho_\lambda(g)a+c(f,g))
\]

on vector fields plus density coefficients. The density ideal is abelian, but generally not central. The formulas specify an algebraic extension; selecting it as a physical model would require additional data.

### 40.2 Integration is a map of coefficient modules and cochain complexes

For weight one,

\[
I(\rho_1(f)a)=I(fDa+(Df)a)=I(D(fa))=0.
\]

Therefore \(I:\mathcal F_1\to\mathbb C\), where the target has trivial action, is a module map. Applying \(I\) to the values of any continuous cochain commutes with the Chevalley–Eilenberg differential. This is an explicit comparison of **density-valued and scalar Lie cochains**.

For the representatives above, integration by parts gives

\[
I(c_1(f,g))=2I((D^3f)g),\qquad
I(\overline c_1(f,g))=0.
\]

Let \(b(X_f)=I(f)/24\). With trivial coefficients \(db(f,g)=-b([f,g]_D)\), hence

\[
db(f,g)=\frac1{12}I((Df)g),\qquad
\boxed{\ \omega=\frac1{24}I\circ c_1-db\ }.
\]

The right side is exactly the normalized Virasoro cocycle \(I(((D^3-D)f)g)/12\) in §36. Thus the density class maps to \(24[\omega]\), while its displayed companion has zero image. The earlier two-mode nontriviality argument for \(\omega\) in turn proves that this \(c_1\) cannot be a coboundary.

Integration on other weights does not give this module map in general:

\[
I(\rho_\lambda(f)a)=(\lambda-1)I((Df)a).
\]

The comparison therefore depends on the weight-one coefficient module. It is not an identification with the cyclic cochain complex of the rotation algebra in §31.

### 40.3 Coverings explain the central factor

In phase coordinates write \(J_kf(\theta)=f(k\theta)\). The vector-field lift and natural density pullback are

\[
i_kf=\frac1kJ_kf,\qquad
j_{k,\lambda}a=k^\lambda J_ka.
\]

The density power uses \(k>0\). These satisfy

\[
[i_kf,i_kg]_D=i_k[f,g]_D,\qquad
\rho_\lambda(i_kf)j_{k,\lambda}a=j_{k,\lambda}\rho_\lambda(f)a.
\]

Transport by \(h\) gives the same maps in the physical coordinate. Since \(D^r(i_kf)=k^{r-1}J_kD^rf\), the five cocycles have precisely specified covering factors:

\[
\begin{aligned}
c_\lambda(i_kf,i_kg)&=j_{k,\lambda}c_\lambda(f,g),&&\lambda=1,2,\\
\overline c_\lambda(i_kf,i_kg)&=k^{-1}j_{k,\lambda}\overline c_\lambda(f,g),&&\lambda=0,1,2.
\end{aligned}
\]

In particular the extensions by \(c_1\) and \(c_2\) themselves admit the compatible maps \((f,a)\mapsto(i_kf,j_{k,\lambda}a)\). Other representative choices require their stated rescalings. Covering compatibility is not unique to a scalar central extension.
...
because averaging over a full phase period preserves \(I(J_ka)=I(a)\). Integrating the \(c_1\) covering identity therefore gives the factor \(k\) for the cubic scalar cocycle. The linear coboundary then recovers the precise \(k\omega\) correction and lift constants of §36.3. This derives the degrees \(13\) and \(39\) through actual maps, including their composition through the factor \(3\).

### 40.4 Finite quadrature: exact within a frequency range, false globally

The occupation integral is continuous in the smooth topology but is not determined by finitely many jets at finitely many points. A smooth bump supported away from those points has all the sampled jets zero and can have nonzero integral. This already disproves S43's equivalence between continuity and finite-point jet dependence. Local differential cochains, integrated local expressions and arbitrary continuous cochains must be distinguished.

The distinction also matters computationally. Consider the uniform \(q\)-point phase average

\[
I_q(f)=\frac1q\sum_{j=0}^{q-1}f(h^{-1}(j/q)).
\]

On modes, \(I_q(e_n^h)=1\) if \(q\mid n\), and zero otherwise. Nonzero multiples of \(q\) therefore alias to the constant mode. Replacing \(I\) by \(I_q\) in the formula for \(\omega\) can fail even antisymmetry. With \(q=39\),

\[
\omega_{39}(e_1^hD,e_{38}^hD)=0,
\qquad
\omega_{39}(e_{38}^hD,e_1^hD)=\frac{9139}{2}.
\]

These are uniform sample points, not the original bin indicators. Antisymmetrizing the sampled expression does not generally repair Jacobi either.

On the other hand, for inputs with Fourier indices bounded by \(B\), \(q>3B\) makes the phase average exact on every product in the trilinear cocycle identity. For example \(q=39\) safely checks all such mode triples with \(B=12\). This follows from the exact geometric sum: a total frequency of absolute value less than \(q\) cannot be a nonzero multiple of \(q\). Derivatives preserve frequencies. Brackets may double the input bound, which is why the three-input condition is stated explicitly.

This is a reproducible quadrature condition for finite fixtures, not a finite-dimensional Lie algebra: brackets can leave the chosen frequency range. It preserves the original distinction between useful finite computations and an exact finite predictive closure.

## 41. Selected corrections to the surrounding survey

The source/change register locates the larger set of consequential corrections in S43. The examples below supply short derivations or reproducible computations for several claims close to the new material. They are audit examples, not identifications of the screen with every algebra named in the source.

### 41.1 Positive Witt cohomology and its actual Laplacian

Let \(L_1\) have basis \(e_i\), \(i\ge1\), and bracket \([e_i,e_j]=(j-i)e_{i+j}\). The algebraic positive-mode algebra and its formal completion should be distinguished; the complete finite weight spaces used here agree. It is not nilpotent in finitely many commutator steps:

\[
(\operatorname{ad}e_1)^r e_2=r!e_{r+2}\ne0.
\]

Its commutator subalgebra is spanned by \(e_i\) for \(i\ge3\), so \(H^1(L_1;\mathbb C)\) already has dimension **two**, with the dual classes of \(e_1,e_2\). The source's dimension-one statement fails at this first degree.

Goncharova's theorem has one class at each of the two generalized pentagonal weights

\[
p=\frac{3q^2-q}{2},\quad \frac{3q^2+q}{2},\qquad q\ge1,
\]

and \(H^0=\mathbb C\). See [Millionshchikov's presentation of the theorem](https://nomc.math.tsu.ru/company/partners/Millionshchikov.pdf). This is a cohomological weight statement, not an assertion that every Verma determinant vanishes only at those levels.

For dual generators \(\omega_k\), the declared CE sign gives

\[
d\omega_k=-\sum_{i<j,\ i+j=k}(j-i)\omega_i\wedge\omega_j.
\]

Writing an all-ordered-pairs sum requires a factor \(1/2\). From this differential the verification constructs complete weight complexes through weight 16. It finds, in degrees one through three, exactly the pairs \((1,2)\), \((5,7)\), \((12,15)\), with one-dimensional classes at each listed weight. This finite computation supports the examples and checks the implementation; the general theorem remains a cited result.

The source's integer-spectrum assertion is supported by the metric-specific theorem recorded in [Fuchs–Wilmarth, Theorem 2](https://msp.org/pjm/2010/247-2/pjm-v247-n2-p.pdf). Its claim that ordinary wedge monomials diagonalize the Laplacian is wrong. With the \(e_i\) orthonormal, degree two and weight eight have matrix

\[
\Delta=\begin{pmatrix}46&12&6\\12&32&12\\6&12&14\end{pmatrix}
\]

in the basis \((\omega_1\wedge\omega_7,\omega_2\wedge\omega_6,\omega_3\wedge\omega_5)\). Its eigenvalues are \(8,28,56\), but every off-diagonal entry is nonzero. The correct spectrum does not validate the incorrect claimed eigenbasis. The separate assertion about universal Fibonacci stabilization in finite-dimensional quotients is not adopted as a settled theorem; the register records the conflicting status in the cited announcement and later account.

### 41.2 Verma and vacuum determinants are different

The Lee–Yang vector in §37 remains correct. S43's new explanation confuses its universal vacuum module with the larger Verma module. In a Verma module with highest weight \(h\), the level-two Gram matrix in the basis \((L_{-1}^2v,L_{-2}v)\) is

\[
G_2(c,h)=\begin{pmatrix}4h(1+2h)&6h\\6h&4h+c/2\end{pmatrix},
\qquad
\det G_2=2h\bigl(16h^2+(2c-10)h+c\bigr).
\]

It vanishes at \(h=0\) for every \(c\), because the Verma module already contains the level-one singular vector \(L_{-1}v\) and its descendants. The universal vacuum module quotients that submodule out. Its level-two form is instead the one-by-one matrix \((c/2)\), nonzero at \(c=-22/5\).

In the vacuum basis \((L_{-4}\mathbf1,L_{-2}^2\mathbf1)\), the level-four form is

\[
G_4^{\mathrm{vac}}(c)=\begin{pmatrix}5c&3c\\3c&c(c+8)/2\end{pmatrix},
\qquad \det G_4^{\mathrm{vac}}=\frac{c^2(5c+22)}2.
\]

At \(c=-22/5\) its kernel has coefficients \((-3/5,1)\), agreeing with §37. These formulas follow directly from mode commutators; the general vacuum/Verma distinction is studied by [Gorelik–Kac](https://arxiv.org/abs/math-ph/0606002).

A first singular-level determinant zero need not be simple either. At \(c=1\),

\[
\det G_2(1,h)=2h(4h-1)^2.
\]

At \(h=1/4\), the level-one determinant is \(1/2\), while the first zero at level two is double. Coincident determinant factors must not be discarded.

### 41.3 Filtration, dimension and quotient conditions

For an ordinary characteristic-zero finite W-algebra, the PBW theorem gives \(\operatorname{gr}W=\mathbb C[S_e]\), a polynomial domain. Nonzero leading symbols multiply to a nonzero symbol, so \(W\) itself is a domain. This proves the statement in every case covered by that theorem; it is not restricted to experimentally checked orbits. It does not transfer unchanged to super PBW rings with exterior generators. See [Gan–Ginzburg](https://arxiv.org/pdf/math/0105225).

The adjective “finite” in finite W-algebra does not say it is a finite-dimensional vector space. For ascending Jordan block lengths \(p_1\le\cdots\le p_n\), the ordinary centralizer dimension is

\[
\dim\mathfrak{gl}_N^e=\sum_{i,j}\min(p_i,p_j)
=\sum_{i=1}^n(2(n-i)+1)p_i.
\]

For \((p_1,p_2)=(1,2)\), it is \(5\), not the source's \(7\). Directly, the commutator map by \(E_{12}\) on three-by-three matrices has rank four and thus kernel dimension five. The alternative coefficients \(2i-1\) require descending block order.

Likewise, if an ideal \(J\) does not annihilate a finite-dimensional module \(V\), then \(V\) does not descend to a module for the quotient algebra. If it does descend, its vector-space dimension is unchanged. Failure of a Yangian truncation condition cannot make that same module infinite-dimensional. The register supplies an exact shifted odd-reflection example and preserves the verified recent presentation results within their actual hypotheses. No Yangian action on the predictive phases is supplied by those reference results.

### 41.4 A small bimodule calculation catches a wrong coefficient ring

Hochschild homology with coefficients in an \(R\)-bimodule \(M\) uses

\[
HH_i(R,M)=\operatorname{Tor}^{R^e}_i(R,M),\qquad R^e=R\otimes R^{\mathrm{op}}.
\]

The two actions matter. For \(R=\mathbb C[x]\), a resolution over \(R^e=\mathbb C[x,y]\) has differential multiplication by \(x-y\). Tensoring with the diagonal bimodule sets \(x=y\), so the resulting differential is zero and \(HH_1(R,R)=R\,dx\ne0\). The source's alternative \(\operatorname{Tor}^R_1(R,R)\) is zero and therefore cannot be the same calculation. The bimodule construction used for link homology is explicit in [Khovanov](https://arxiv.org/pdf/math/0510265).

In the associated rank-one example \(B=\mathbb C[x,y]/(x^2-y^2)\), a bimodule map \(R\to B\) must send \(1\) to an element killed by \(x-y\). The source's proposed image \(1\) fails. A suitable normalized image is \((x+y)/2\), since \((x-y)(x+y)=0\) in \(B\). This is a direct test of the two module actions, independent of a diagram's appearance.

The later knot and Soergel material remains reference-only. No link diagram, Coxeter realization, parabolic data or comparison functor is supplied for the screen. Correcting a diagram formula does not create those missing objects or produce a new knot invariant of the toroidal model.

## 42. v1.6 source review, verification, and resulting status

The new source is **S43, `Mathamatics5 .txt`**: 241,767 bytes, 3,252 lines and 32,254 whitespace-separated words. The filename includes a space before the extension. Its SHA-256 is

~~~text
6ea07265b8f9be872adaff518e168546c41c66e8f59df8eec1a15ce0549114b6
~~~

The opening assessment of v1.5 states that it lacked the verifier and archive. It is not counted as an independently executed package receipt. This update replays the actual v1.5 verifier in a temporary copy, including its nested v1.4 replay, and reproduces the stored v1.5 receipt byte for byte. That receipt's SHA-256 is

~~~text
398d0a30262c57811d8e8f3730ef699887713ab8118da551c9dc18c7b148fbc3
~~~

The complete new text was read across the mathematical review. The audit concentrates on the explicit extensions, consequential formula and category errors, and the primary results used in the new report. It does not certify every classification, historical attribution, stabilization claim or broad categorical equivalence. Source-file instructions remain source content. The separate source/change register identifies the relevant passages and references.

| Executed check | Scope and result |
|---|---|
| Density representation | A polynomial identity in three arbitrary Fourier indices and a symbolic density weight vanishes identically |
| Density cocycles | Five independent three-variable polynomial CE identities vanish identically |
| Finite Laurent expressions | 40 triples, seven rational density weights, module action, invariant pairing, integration, covering factors and orthogonal projections |
| Integration comparison | The density-valued cocycle yields exactly the cubic scalar term; the explicit coboundary yields the normalized Virasoro representative; the companion integrates to zero |
| Quadrature | All 15,625 triples in the mode range −12 through 12 satisfy the 39-point condition exactly; higher-mode examples exhibit both antisymmetry and Jacobi failures |
| Smooth coordinates | Three sinusoidal coordinate changes, six real/complex density weights and 1,024 points per case verify transported time and Lie actions and the weighted norm; maximum measured errors are below \(5\times10^{-15}\) |
| Positive Witt complexes | 85 bidegrees through weight 16 and degree four; 102 differential matrices and 85 square-zero compositions, with exact ranks and the displayed low-degree classes |
| Laplacian | The weight-eight degree-two matrix is constructed from the CE differential; its characteristic polynomial has roots 8, 28 and 56 |
| Verma and vacuum forms | 100 symbolic commutator fixtures, exact low-level Gram matrices, the Lee–Yang null vector, and the double first zero |
| Centralizer dimension | The actual nine-by-nine commutator operator has rank four and kernel dimension five |
| Odd reflection | The cited rational transformation is checked with its spectral shifts; 16 exact supercommutators verify a separate two-dimensional highest-weight example |
| Prior replay | v1.5 and its nested v1.4 execution reproduce the preserved receipts exactly |

All included checks pass. Integer/rational calculations and symbolic identities are separated from floating-point coordinate fixtures. The latter are implementation checks of analytical identities, not interval-arithmetic proofs. Finite cohomology computations use complete homogeneous weight complexes; they are not an assertion about the cohomology of a finite-dimensional quotient. External general theorems remain cited results, not consequences of numerical sampling. No empirical simulation in the original packet was rerun.

The verification archive includes the full report, its source/change register, the S43 snapshot, three standard-library verification modules, the executed receipt, the source inventory and checksums. It preserves the **entire v1.5 package**, including all earlier versions:
...
| v1.4 | `v1_5/v1_4/` |
| v1.3 | `v1_5/v1_4/v1_3/` |
| v1.2 | `v1_5/v1_4/v1_3/v1_2/` |
| v1.1 | `v1_5/v1_4/v1_3/v1_2/v1_1/` |

From the extracted archive, using Python 3.10 or later, run:

~~~text
python verify_v16.py
~~~

The new verifier and its two companion modules require only the standard library. Previous verifiers execute in temporary copies, leaving their packaged files intact. Packaging extracts the archive, reruns the new command, checks the reproduced receipt and verifies all nested checksums. Versions v1.1–v1.3 retain their historical execution records; their own mathematical checks are not described as newly executed here.

The inventory covers **45 explicitly supplied input files**, plus the two earlier checksum dependencies. Issued prior artifacts and source snapshots are preserved. Hashes identify the reviewed bytes and detect changes; they are not an external certification of the source's mathematical truth.

The completed update connects density transformation laws with the established conformal measures, gives time-compatible weighted spaces and projections, and supplies an explicit integration map to the existing Virasoro cocycle. Covering factors now follow through that map, and the finite-quadrature limit is quantified. Predictive phase requirements, exact slip laws, non-mixing and the qualified toroidal conclusions retain their established scope. No new physical central charge, knot invariant or unconstructed representation is inferred.
