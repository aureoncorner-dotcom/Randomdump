...
Status
experimental operational extension; eligibility, response fidelity, proof-status firewall, typed return-residual closure, and exact finite worked specimens
Profile
set core plus witness, seam, typed constraint/route, eligibility, response, window, independent-coder audit, typed return-residual record, and finite orbit-quotient examples
Date
2026-09-02
Attribution
family-built
License
append-only successor to GQG Core Card v0.12; v0.5-v0.12 remain preserved predecessors
Continuity
preserve v0.12 unified return-residual closure while adding exact orbit-quotient witnesses, driven residual cocycles, and a concrete residual-descent counterexample
Purpose
retain the complete constraint-response instrument and typed unified return-residual record while demonstrating, in the chart/formal namespace, how exact quotient descent can coexist with failed residual-only autonomy


Versioning rule
GQG Core Cards v0.5-v0.12 remain preserved predecessors. v0.13 retains the v0.12 word-labeled empirical instrument, typed unified return-residual record, descent test, proof-status firewall, and independent-coder gate. It adds two exact finite worked specimens and a typed formal-register firewall. No v0.13 classification is back-ported without an explicit supersession edge.


Claim boundary
GQG v0.13 keeps theorem geometry, chart/formal constructions, dyadic admissibility, retained-context comparison, and empirical instrumentation separately typed; defines eligibility before denominator formation; distinguishes standing, event response, window binding, and derived demotion; and treats progressive/regressive/distorted return as a derived interpretation rather than a raw state. Exact finite quotients do not validate a hidden platform geometry. GQG v0.13 does not infer a hidden actor, motive, implementation, occupant, normative polarity, statistical independence, universal threshold, common metric, joint physical state space, or autonomous residual map without descent. Unknown, partial, excluded, and censored states remain explicit.
...
65. v0.12 governing extension: unified return-residual record
v0.12 preserves the v0.11 theorem/dyadic/empirical namespace firewall and adds a companion record for comparing loop returns across three separately typed channels. The companion is a record product. It is not a claim that chart space, dyadic admissibility, retained context, and the empirical instrument are one mathematical geometry.

Let the joint loop record be
...
$$
x_n\in\mathcal X_C,\qquad d_n\in\mathcal X_D,\qquad k_n\in\mathcal K.
$$

Here $\mathcal X_C$ is a declared chart/atlas state space, $\mathcal X_D$ is a declared dyadic encounter state space, and $\mathcal K$ is a declared retained-context state space. Their product is bookkeeping only unless an application declares additional structure.

A state update may be written

$$
S_{n+1}=\Phi_n(S_n),
$$

with component updates such as

$$
x_{n+1}=H_{\gamma_n}(x_n),\qquad
 d_{n+1}=T_{D,n}(d_n),\qquad
 k_{n+1}=G_n(k_n,x_n,d_n).
$$

The route $\gamma_n$, temporal mode, or context update may vary with $n$. Stationarity is not assumed.

66. Typed residual comparisons
Each channel declares its own comparison map:

$$
\operatorname{Cmp}_C:\mathcal X_C\times\mathcal X_C\to\mathcal R_C,
$$

$$
\operatorname{Cmp}_D:\mathcal X_D\times\mathcal X_D\to\mathcal R_D,
$$

$$
\operatorname{Cmp}_K:\mathcal K\times\mathcal K\to\mathcal R_K.
$$

Define

$$
\delta^C_{n+1}=\operatorname{Cmp}_C(x_n,x_{n+1}),
$$

$$
\delta^D_{n+1}=\operatorname{Cmp}_D(d_n,d_{n+1}),
$$

$$
\delta^K_{n+1}=\operatorname{Cmp}_K(k_n,k_{n+1}).
$$

The unified return-residual record is

$$
\boxed{R_{n+1}=(\delta^C_{n+1},\delta^D_{n+1},\delta^K_{n+1}).}
$$

Subtraction is permitted only when the relevant channel separately supplies an additive structure and the declared comparison rule is $\operatorname{Cmp}(a,b)=b-a$. The word residual does not manufacture a vector space, norm, metric, sign, magnitude, or common scale.

67. Residual autonomy is a descent claim
The notation
...
is not assumed. It is earned only if the state-pair dynamics descend through the residual observation.
...
and define the residual observation

$$
q_R(Z_n)=R_n.
$$

Let the state-pair update be

$$
\Psi_n(S_{n-1},S_n)=(S_n,\Phi_n(S_n)).
$$

A residual-level map exists on the declared domain only when
...
for every state pair $z,z'$ identified by the residual observation. If this descent condition holds, there is a well-defined factor

$$
F_{R,n}:\operatorname{im}(q_R)\to\operatorname{im}(q_R)
$$
...
Only then may the residual dynamics be written autonomously as

$$
R_{n+1}=F_{R,n}(R_n).
$$

Failure of descent is itself a retained finding: two state pairs share the same current residual but produce different next residuals. The residual representation has then collapsed a distinction that remains dynamically relevant. The repair is to refine the retained state or residual witness, not to force closure.

68. Dyadic residual direction without a scalar score
For the dyadic channel, let

$$
v_n=F_D(d_n)=(R_{\mathrm{contact}},C_{\mathrm{bind}},U_{\mathrm{exit}},N_{\mathrm{third}})_n.
$$

For fully resolved binary coordinates, define the componentwise order

$$
v\preceq w\iff v_i\le w_i\ \text{for every coordinate }i.
$$

Then the coordinate-direction record is:

- `UP` when $v_n\prec v_{n+1}$;
- `DOWN` when $v_{n+1}\prec v_n$;
- `FLAT` when $v_n=v_{n+1}$;
- `MIXED` when neither resolved vector dominates the other; and
- `UNRESOLVED` when a coordinate required for the comparison is `UNKNOWN`.

This is a partial-order read, not a scalar coherence score. Separately retain

$$
A_n=\operatorname{Adm}_D(d_n).
$$

For resolved gates, record $0\to1$ as `OPENING`, $1\to0$ as `CLOSURE`, $1\to1$ as `STABLE_OPEN`, and $0\to0$ as `STILL_INADMISSIBLE`. An `UNKNOWN` gate remains unresolved.

69. Empirical provenance join; no residual-to-demotion promotion
The return-residual record does not replace the v0.11 empirical instrument. For a named constraint $c$, the governing event record remains

$$
X_c(t)=(\mathsf{Object},\mathsf{Standing},\mathsf{Response},\mathsf{Route})
$$

with eligibility $\chi_c(t)$ decided before denominator formation. Demotion remains a derived sequence- or window-level classification under a frozen rule.

A residual episode may carry provenance links to one or more $\mathcal I_c$ records:

$$
J_n:R_n\rightsquigarrow\{X_c(t)\text{ records located in episode }n\}.
$$

The arrow above is an index/provenance relation, not a mathematical implication. No chart residual, dyadic gate transition, or retained-context label may be relabeled as empirical constraint demotion without the v0.11 eligibility, response, window, and confirmation requirements.

70. Directionality and residual interpretation
Movement does not determine polarity. Attach a declared sidecar when a progressive/regressive/distorted interpretation is attempted:

$$
P_n=(\text{constraint source},\text{constraint standing},\text{change standing},\text{route source},\text{route holder},\text{route beneficiary}).
$$

A return interpretation is therefore a function of retained residual structure plus declared directionality and provenance:

$$
\mathfrak R_n=(R_n,P_n,J_n).
$$

Recommended output vocabulary:

- `PROGRESSIVE`: resolved evidence of nondegrading/opening dyadic movement or stable admissibility, authorized or appropriately held directionality, and no conflicting retained-context degradation under the declared comparison rule;
- `REGRESSIVE`: resolved dyadic degradation, closure, compulsory/third-held route capture, or declared context degradation under the application rule;
- `DISTORTED`: genuinely mixed or cross-channel contradictory movement, including a resolved `MIXED` dyadic transition or declared oscillatory/skewed return;
- `UNCHANGED`: exact/flat return under all comparison rules required by the application; and
- `UNRESOLVED`: any classification-relevant coordinate is unknown, censored, untyped, or lacks required provenance.

These labels are derived interpretations, not new theorem states and not empirical macrostates.

71. Historical constraint-status projection boundary
Historical v0.8-v0.10 $D^-$, $A^+$, $L^+$, and $D^+$ labels may be attached only as version-qualified projections from their required historical coordinates. They do not govern new v0.12 empirical records.

In particular:

$$
L^+\text{ requires }(B_{\mathrm{after}},E_{\mathrm{after}})=(\mathrm{NO},\mathrm{NO}).
$$

A state $(\mathrm{NO},\mathrm{UNKNOWN})$ is `UNRESOLVED`, not $L^+$. Historical $A^+_{\mathrm{cand},0.8}$ remains distinct from resolved $A^+_{\mathrm{res},0.9+}$.

72. Formation rules appended in v0.12
Rules 1–68 remain versioned historical receipts. Append:

69. **Typed residual-product rule.** A unified return-residual record may carry multiple typed channels together without identifying their state spaces, proof status, scales, or mechanisms.
70. **Declared-comparator rule.** Every residual channel declares the comparison map that produces its residual. Subtraction, sign, magnitude, norm, and metric are unavailable unless separately supplied by that channel.
71. **Residual-descent rule.** Autonomous $R_{n+1}=F_R(R_n)$ dynamics may be claimed only after the state-pair update descends through the residual observation.
72. **Residual-refinement rule.** Failure of descent is evidence that the current residual witness erased a dynamically relevant distinction; refine the retained record rather than forcing closure.
73. **Dyadic-partial-order rule.** Coordinate improvement/degradation is retained as `UP`, `DOWN`, `FLAT`, `MIXED`, or `UNRESOLVED`; no coordinatewise movement is silently converted into a scalar coherence score.
74. **Empirical-provenance rule.** Residual records may link to $\mathcal I_c$ events but may not substitute for eligibility, standing, response, route, window inference, or independent confirmation.
75. **Residual-polarity rule.** Progressive/regressive/distorted labels require a declared directionality/polarity rule; topology alone does not determine normative direction.
76. **Historical-projection rule.** Legacy $D^-/A^+/L^+/D^+$ labels remain version-qualified projections and may not govern v0.12 word-labeled empirical records.

73. Operational diagnostics added in v0.12
**GQG-E746 · Residual-space collapse.** Raise when typed chart, dyadic, retained-context, or empirical channels are treated as one geometry merely because they are stored in one residual record.

**GQG-E747 · Undeclared subtraction.** Raise when a residual is written as $b-a$ without a declared additive structure and comparison rule for that channel.

**GQG-E748 · Residual-autonomy promotion.** Raise when $R_{n+1}=F_R(R_n)$ is asserted without a descent test showing that equal present residuals imply equal next residuals under the state-pair update.

**GQG-E749 · Residual-closure forcing.** Raise when failure of residual descent is hidden by averaging, relabeling, or discarding the state distinctions that break closure.

**GQG-E750 · Dyadic-score leakage.** Raise when one improving admissibility coordinate is reported as overall dyadic improvement despite a simultaneous degradation or unresolved coordinate.

**GQG-E751 · Residual-demotion substitution.** Raise when a residual, gate transition, or progressive/regressive label is relabeled as empirical demotion without the v0.11 eligibility/window rule.

**GQG-E752 · Topology-polarity residual leak.** Raise when `UP`, `DOWN`, `MIXED`, or a historical macrostate is treated as sufficient proof of progressive, regressive, helpful, harmful, opening, or containing direction without the declared sidecar.

**GQG-E753 · Historical-cell completion by UNKNOWN.** Raise when a legacy resolved cell such as $L^+$ is assigned by treating `UNKNOWN` as `NO`.

74. Minimal v0.12 unified return-residual record
```yaml
gqg_version: 0.12
inherits: 0.11
unified_return_residual:
  ...
  state_before:
    chart_state_id: null
    dyadic_state_id: null
    retained_context_id: null
  state_after:
    chart_state_id: null
    dyadic_state_id: null
    retained_context_id: null
  ...
  empirical_links:
    ...
    independent_confirmation_status: not_evaluated
    historical_projection_version: null
    historical_projection: null
  classification:
...
75. v0.12 acceptance target
A conforming v0.12 implementation must satisfy every still-applicable v0.11 acceptance target and additionally:

- keep chart, dyadic, retained-context, and empirical namespaces typed even when stored in one record;
- declare the comparator used by every residual channel;
- refuse autonomous residual dynamics unless the state-pair update passes the residual descent test;
- preserve counterexamples to residual closure and use them to refine the retained witness;
- separate dyadic coordinate direction from the admissibility gate and from progressive/regressive interpretation;
- link empirical records by provenance rather than substituting residual labels for $\mathcal I_c$ coordinates;
- preserve `UNKNOWN` and censoring across residual and historical-projection reads; and
- version-qualify every legacy constraint-status projection.

76. Compact laws appended in v0.12
One record may carry three residual channels; one record does not make one geometry.
Residual means declared comparison, not automatic subtraction.
State dynamics first; residual dynamics only after descent.
Equal residuals that diverge on the next loop expose a missing retained distinction.
Dyadic coordinate direction != scalar coherence != normative polarity.
Residual topology != empirical demotion.
Progressive/regressive/distorted are derived reads, not raw states.
UNKNOWN remains unresolved.

77. v0.12 provenance and version seal
Inherited without modification at the predecessor level: all v0.11 geometry-namespace, eligibility, response/window/demotion, historical-rate, proof-status, and independent-coder requirements; all v0.10 temporal-address and route-return separation; and all earlier witness/transition-fidelity requirements.

New in v0.12: typed unified return-residual record, declared channel comparators, state-pair residual observation, residual-closure descent criterion, closure-counterexample retention, dyadic partial-order direction, empirical provenance join, residual-polarity firewall, legacy-cell UNKNOWN repair, rules 69–76, diagnostics GQG-E746 through GQG-E753, and the v0.12 execution record.

Not claimed: no common metric or vector structure across residual channels; no autonomous residual dynamics without descent; no empirical demotion from a residual label; no normative polarity from dyadic topology; no causal mechanism from residual closure or nonclosure; and no proof-status transfer from the multiplication representation.

Version seal: v0.12 succeeds v0.11 without rewriting history. It adds a typed return-residual companion and a test for when residual dynamics themselves are well-defined.


78. v0.13 governing extension: orbit quotients and residual cocycles
v0.13 adds worked examples inside the chart/formal namespace. They instantiate the existing quotient and residual-descent rules; they do not alter the dyadic namespace, create an empirical event, or lend theorem status to a platform hypothesis.

Narrow v0.13 precision correction to inherited §67: a descended factor written $F_{R,n}$ is a time-$n$ factor and is generally nonautonomous. A single autonomous law $R_{n+1}=F_R(R_n)$ requires a common domain and one factor $F_R$ that works at every time, equivalently $F_{R,n}=F_R$ throughout that domain. For a stationary state-pair update $\Psi$, require

$$
F_R\circ q_R=q_R\circ\Psi.
$$

This clarification supersedes only the inherited use of *autonomously* for an $n$-indexed family; the v0.12 descent condition and record remain historical receipts.

The weave example is conditional on the source construction's own requirement: the two six-step rails must first receive a separately declared phase correspondence and be represented by one shared $C_6$ coordinate. Only inside that declared construction is the following product available.

79. Exact binary orbit quotient in the 24-state weave
Let
...
$H_\Delta\cong C_{12}$. Define the typed orbit witness
...
The map is a well-defined surjective homomorphism. Moreover,

$$
H_\Delta\subseteq\ker\chi_{\mathrm{orb}},
$$

and both sets have $12$ elements. Therefore
...
The $24$ formal states are exactly two diagonal $H_\Delta$-orbits of $12$ states. A nominal diagonal trajectory occupies one orbit; the complementary orbit is formal but inaccessible until the update has a declared quotient-changing component.

For $u_n\in G_{24}$, let a declared source-level deviation $r_n^{(24)}\in G_{24}$ modify the nominal step:

$$
u_{n+1}=u_n+h+r_n^{(24)}.
$$

Type the sector-switch residual by
...
The value $\eta_n^{\mathrm{orb}}=0$ preserves the orbit; $\eta_n^{\mathrm{orb}}=1$ crosses it. The complete deviation $r_n^{(24)}$ remains part of the source record because the quotient bit does not identify the complete operator.

80. Exact $C_3$ strand quotient and typed slip residual
Work in a separate screen namespace. Let
...
The driver is one stationary circle rotation on the common domain $\mathbb R/\mathbb Z$:
...
Use $j_n^{(39)}$, not the inherited dyadic symbol $d_n$, for the declared screen-jump representative:
...
The quotient map and strand label are
...
This channel explicitly chooses $j_n^{(39)}\in\{14,15\}$ as integer representatives and declares the comparator

$$
\sigma_n^{(39)}
:=
\operatorname{Cmp}_{39}(b_n,b_{n+1})
=
15-j_n^{(39)}
\in\{0,1\}.
$$
...
The declared representatives and comparator are what license subtraction here; the generic word residual does not. The exact update is

$$
b_{n+1}=b_n+15-\sigma_n^{(39)}\pmod{39},
$$

and quotienting gives
...
Thus $\sigma_n^{(39)}=0$ records a $+15$ same-strand step, while $\sigma_n^{(39)}=1$ records a $+14$ move to the preceding strand.
...
$$
c_\sigma(m,n)+c_\sigma(n,p)=c_\sigma(m,p)
$$

and

$$
s_n^{(39)}=s_m^{(39)}+c_\sigma(m,n).
$$

This is an exact additive $C_3$-valued cocycle over the retained golden-phase rotation. It is driven quotient descent, not an autonomous update of the slip bit.

81. Located residual-descent counterexample
Under the GQG transition convention, set

$$
R_{n+1}^{(39)}:=\sigma_n^{(39)}.
$$

The declared trace contains
...
In the common-domain form of §67,
...
Therefore no time-homogeneous residual-only factor

$$
F_\sigma:\{0,1\}\to\{0,1\}
$$

satisfies $R_{n+2}^{(39)}=F_\sigma(R_{n+1}^{(39)})$ on the declared orbit domain. This is a concrete failure of the stationary-autonomy descent condition—the common-$\Psi$ case of §67.

The counterexample is stronger than a missing strand label:

$$
s_1^{(39)}=s_8^{(39)}=2,
$$

so the current pair $(s_n^{(39)},\sigma_n^{(39)})$ is also equal at $n=1$ and $n=8$ while the next slip differs. The residual witness has erased dynamically relevant golden phase. A valid refinement retains $\theta_n$ itself, or an equivalent sufficient phase state. Retaining only the constant circle increment $[\theta_{n+1}-\theta_n]=[\alpha]$ in $\mathbb R/\mathbb Z$ does not repair the descent failure.

82. Typed formal $72$ register; no geometry promotion
The exact sequence
...
splits through the complement $\langle(3,0)\rangle\cong C_2$. Choosing that splitting, or choosing an origin on each $H_\Delta$-torsor, licenses coordinates

$$
C_{12}^{\mathrm{position}}\times C_2^{\mathrm{orbit}}.
$$

The $C_3$ strand coordinate comes from a different screen construction. Combining it with the weave requires a declared common domain, synchronization, and joint observation map. Only after those declarations may an application store
...
This typed register has $72$ elements—six sectors of twelve. If it carries the direct-product group structure, then
...
so it is not $C_{72}$. A joint register also does not establish joint reachability: when $\eta_n^{\mathrm{orb}}=0$, the $C_2$ coordinate remains fixed.

No identity or evidentiary bridge runs from this typed register to the tested $72$-model bank. The latter cardinality came from model-design enumeration, and its frozen-bank verdict was null. Exact algebra neither rescues that null nor validates a moving-lattice or hidden-platform geometry.

83. Formation rules appended in v0.13
Rules 1–76 remain versioned historical receipts. Append:

77. **Orbit-accessibility rule.** A formal product-state count must be separated from the orbit reachable under the declared update.
78. **Sector-crossing witness rule.** A change of quotient sector requires a named source-level update whose quotient component is nonzero; a sector label alone is not the operator.
79. **Driven-cocycle rule.** An exact quotient-label recurrence driven by retained state is not an autonomous residual law.
80. **Phase-refinement rule.** When residual-only descent fails because phase was erased, retain a sufficient phase value or state; the constant circle increment is not a substitute.
81. **Joint-product rule.** Coordinates from distinct constructions may be combined only after declaring a common domain, synchronization, and joint observation map.
82. **Cardinality firewall rule.** Equal cardinality does not establish isomorphism, common mechanism, empirical identity, or evidentiary support.

84. Operational diagnostics added in v0.13
**GQG-E754 · Formal-state accessibility inflation.** Raise when all states in a formal product are reported as reachable even though the declared update remains in one proper orbit.

**GQG-E755 · Unwitnessed sector crossing.** Raise when a quotient label changes without a named source-level update and a computed nonzero quotient component.

**GQG-E756 · Driven-cocycle autonomy promotion.** Raise when an exact recurrence for a quotient label under retained-state driving is relabeled as autonomous residual closure.

**GQG-E757 · Insufficient phase repair.** Raise when phase-caused nonclosure is said to be repaired by retaining only the constant circle increment while the phase value remains erased.

**GQG-E758 · Equal-cardinality geometry substitution.** Raise when a formal $72$-element register, $C_{72}$, or a $72$-candidate model bank is identified with another merely by its count.

85. Minimal v0.13 orbit-quotient supplement
Version values are quoted in successor YAML so minor-version identity survives parsing. Predecessor records retain their original source formatting as historical receipts and are not the v0.13 serialization contract.

```yaml
gqg_version: "0.13"
inherits: "0.12"
orbit_quotient_specimen:
  namespace: chart_formal
  source_phase_correspondence_declared: false
  construction_status: conditional_on_phase_correspondence
  weave:
    source_group: "C_6 x C_4"
    acting_subgroup: "< (1,1) > ~= C_12"
    quotient_map: "chi_orb(a,b) = a-b mod 2"
    kernel: "< (1,1) >"
    quotient_group: C_2
    formal_state_count: 24
    orbit_count: 2
    reachable_orbit: null
    reachable_state_count_under_nominal_update: 12
    current_orbit_label: null
    source_deviation_id: null
    sector_switch_residual_eta: null
    crossing_operator_witnessed: false
  screen:
    alpha_definition: "alpha = ((1 + sqrt(5))/2)^(-2)"
    driver: "theta_(n+1) = theta_n + alpha mod 1"
    absolute_phase_state_id: null
    screen_state_b: null
    jump_representative_j_39: null
    strand_s_39: null
    slip_residual_sigma_39: null
    quotient_increment_c_3: null
  residual_closure:
    dynamics_mode: driven_cocycle
    stationary_residual_only_map_claimed: false
    descent_status: failed_on_declared_orbit
    closure_status: unresolved
    counterexample_indices: [1, 8]
    refinement_required: absolute_phase_or_equivalent
  formal_register_72:
    formal_state_count: 72
    ...
    identified_with_C_72: false
    identified_with_tested_model_bank: false
  empirical_status: no_platform_geometry_validation
```

86. v0.13 acceptance target
A conforming v0.13 implementation must satisfy every still-applicable v0.12 target and additionally:

- declare each orbit quotient's source group, acting subgroup, quotient map, kernel, and reachable orbit;
- preserve the complete source-level operator when recording its quotient-valued sector residual;
- distinguish a driven quotient cocycle from autonomous residual dynamics;
- retain the driver value or another sufficient refinement when residual-only descent fails;
- require a common domain, synchronization, and joint observation map before multiplying coordinates from distinct constructions;
- test reachability separately from formal cardinality; and
- deny structural or evidentiary identity from equal counts alone.

87. Compact laws appended in v0.13
Twenty-four formal states can be two twelve-state orbits.
Sector crossing requires a witnessed quotient-changing operator.
Exact quotient descent under a driver != autonomous residual dynamics.
Equal slip now and unequal slip next exposes retained phase.
Six sectors of twelve != $C_{72}$ != a $72$-candidate model bank.
Exact finite geometry != evidence for a platform geometry.

88. v0.13 provenance and version seal
Inherited without modification at the predecessor level: the entire v0.12 typed residual record and descent criterion; the v0.11 eligibility/response instrument and proof-status firewall; all earlier witness, seam, temporal-address, constraint-status, and independent-coder requirements.

New in v0.13: the time-indexed-versus-autonomous factor precision correction; the exact $C_2$ weave-orbit witness; the exact $C_3$ screen-strand witness; typed sector-switch and slip residuals; additive quotient cocycles; a located residual-descent counterexample; explicit phase refinement; the conditional formal $72$ register; rules 77–82; diagnostics GQG-E754 through GQG-E758; and the v0.13 supplement record.

Source receipts: `TRIADIC_WEAVE_ULTRA_COMBINATION_HANDOFF_v3_WITNESS_HARDENED.md` supplies the conditional $C_6\times C_4$ product and two twelve-state diagonal orbits; `R4A_GOLDEN_DRAIN_39_SCREEN_RUN_2026-08-25.md` supplies the fixed $39$-screen definitions; `GEOMETRY_EXTENSION_TEST_REPORT_v1.0.md` supplies the null frozen-bank verdict and the lack of empirical support for the formal hierarchy. Its narrower later-cohort missing-output association remained only a non-confirmatory lead requiring prospective testing. That lead used the one-opportunity-lagged arriving source jump $d_{n-1}$; the source-defined departing jump $d_n$, which is the $j_n^{(39)}$/$\sigma_n^{(39)}$ alignment formalized here, was null. The v0.13 derivations sharpen state accounting and falsifiability without changing those receipts.

Not claimed: no hidden actor, controller, platform, moving lattice, physical sector, common clock, empirical demotion, causal mechanism, or autonomous slip law follows from these exact finite calculations.

Version seal: v0.13 succeeds v0.12 without rewriting history. It adds a finite worked example in which quotient-label descent is exact while residual-only autonomy fails, and it preserves that failure as evidence of missing state rather than as support for a geometry hypothesis.
