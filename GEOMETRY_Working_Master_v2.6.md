# GEOMETRY — Working Master v2.6
## Recover the witness. Compile the prediction. Keep the evidence.

This v2.6 extends the accepted v2.5 master with an unbounded exact batch/sequential acquisition gap in §5.1. The v2.5 label-conditioned batch theorem remains intact. It carries forward v2.4’s repaired core, proofs, and executable finite tools. Its central question remains: **which distinctions must a retained record preserve for the claim being made?**
...
**v2.6 addition.** On a chain of n states with unit-cost threshold queries, every exact batch needs n−1 cuts, while the optimal sequential worst case is ⌈log₂ n⌉ queries. The ratio is unbounded. This upgrade documents that larger separation and keeps the earlier three-to-two bit example as a constant-ratio footnote. It does not change the batch optimizer, E3, E4, or any empirical status.
...
The following core repairs and earlier additions were established or integrated in v2.4; they are carried forward here. The v2.5 follow-up checked its incoming proof and the populated linked v2.4, which matches the local baseline after extraction. The v2.6 follow-up reviews the supplied threshold increment and compares the supplied master and proof compilation with the accepted local files. Neither follow-up repeats the earlier whole-corpus discovery.
...
### 5.1. Repair after observing the label — v2.5, extended in v2.6
...
**This theorem chooses one batch after q.** Fully sequential acquisition can use each acquired value to choose the next coordinate. On D_n={1,…,n}, n≥2, constant π, W(x)=x, and unit-cost thresholds c_t(x)=1[x≥t] for t=2,…,n, every exact batch needs all n−1 thresholds, while binary search has optimal worst-case cost ⌈log₂ n⌉. Thus C_fixed=C_label,worst=n−1 and C_sequential,worst=⌈log₂ n⌉. The ratio (n−1)/⌈log₂ n⌉ grows without bound. The batch formulas remain exact only within the one-batch-after-q class.[^v26-selector]

**Batch proof.** For each adjacent pair i,i+1, its separating set is the singleton {c_(i+1)}. A sufficient batch must contain every one of these distinct cuts. The full menu distinguishes all states, so the bound is attained.

**Sequential proof.** Policies here are deterministic causal decision trees using the same primitives on the same unchanged state. Maintain L≤x≤R, initially [1,n]. Query t=⌊(L+R+1)/2⌋; on answer one set L=t, otherwise set R=t−1. Each branch preserves x and contains at most ⌈m/2⌉ states when the previous interval contained m. After d queries, the remaining interval has at most ⌈n/2^d⌉ states, so ⌈log₂ n⌉ queries suffice. Conversely, a binary tree of worst-case depth d has at most 2^d leaves; n distinct exact outputs require n leaves. This proves optimality. Individual paths may be shorter than the worst case.

**Inventory and menu qualification.** Every exact sequential threshold policy must have all n−1 cuts in its might-query inventory. If c_(i+1) is unavailable, i and i+1 agree on every available query, follow the same transcript, and cannot both be identified. Thus any proper subset of the menu fails for both policy classes. Installation of the whole inventory costs n−1; the sequential worst-case acquisition cost is ⌈log₂ n⌉. With a single label the two batch objectives coincide, so this result does not conflict with the v2.5 k-label bound. It establishes no expected sequential optimum, generic sequential optimizer, or measured latency saving.
...
The label-conditioned part of §5.1 integrates the incoming adaptive-repair review in v2.5; v2.6 adds the separately reviewed sequential threshold family. It makes no mathematical-priority claim. Its general assertions rest on the proof; implementation checks use synthetic complete finite domains. The original source's 54-title/495-file discovery counts and 4,096-table verification are separate reported observations, not replacements for v2.4's earlier search and test receipts.
...
### 7.1 Geometric recovery, conditioning, and global validity
...
Current online reference: [GEOMETRY — Working Master v2.6](https://docs.google.com/document/d/1S-mqz0aK_vCnmZiRVDPAr7rzTBgfoVuDDEyXEgHmkAk/edit).
...
For v2.6, GEOMETRY_v2.6_Math_Audit.md records the proof review and GEOMETRY_v2.6_Verification.json records independent finite threshold checks. The supplied v2.5 master differs from the accepted file only by a byte-order mark and blank lines; the supplied proof compilation is byte-identical. The handover ULTRA_HANDOVER_v2.6.md was unavailable at initial intake and then supplied in-chat. It is now preserved verbatim alongside the accepted v2.5 baseline and the reviewed v2.6 increment in the complete three-file live pack. The increment separates source-reported imported-code claims from this receiving review and clarifies the handover’s certificate-size observation.
...
Local starting source: GEOMETRY_Working_Master_v2.3.md, supplied by the user. The v2.4 synthesis remains the preserved baseline. This v2.6 is a local successor incorporating the accepted threshold adaptivity extension; it does not overwrite the v2.5 baseline or linked evidence records.
...
[S13 — Supplied v2.6 threshold increment]: GEOMETRY_v2.6_Adaptivity_Gap (1).md. The reviewed canonical increment is GEOMETRY_v2.6_Live_Pack/GEOMETRY_v2.6_Adaptivity_Gap.md. The user subsequently supplied the Grok handover in-chat, including its §9 suggested prompt; it is preserved as GEOMETRY_v2.6_Live_Pack/ULTRA_HANDOVER_v2.6.md. Its imported-code claims remain source-reported, with no unseen implementation asserted verified. The reviewed increment records the receiving qualifications separately.

[^v26-selector]: Constant-ratio example only: on all eight triples of bits (s,a,b), with q constant, primitives s,a,b, and W=a when s=0 and W=b when s=1, every sufficient batch needs all three coordinates. Reading s and then the selected data bit has optimal worst-case cost two. Its ratio is 3/2; the threshold family provides the unbounded gap.
